...
def register_attention_input_hooks(model: torch.nn.Module, module_path: str, layer_start: int = 0, layer_end: int = -1) -> Tuple[ActivationStore, List]:
    store = ActivationStore()
    handles = []
    blocks = iter_transformer_blocks(model)
    total = len(blocks)
    end = total if layer_end < 0 else min(layer_end, total)
    for block in blocks[layer_start:end]:
