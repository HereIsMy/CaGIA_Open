                ...
                try:
                    ...
                    if is_oom(inner_error):
                        ...
                        try:
                            ...
                            print(f"[OOM] 当前显存: used={used:.2f}GB / total={total:.2f}GB, free={free:.2f}GB")
                        except Exception:
                            pass
                        return
                    raise
...
def linear_reconstruct(
    ...
    for i, m in enumerate(matrices):
        if hasattr(m, 'shape') and m.ndim >= 2:
            try:
                rank = int(torch.linalg.matrix_rank(m.detach().float().cpu()).item())
                norm = float(m.detach().float().norm().item())
                print(f"[DEBUG linear_reconstruct] matrix[{i}] shape={m.shape}, rank={rank}, norm={norm:.6e}")
            except Exception as e:
                print(f"[DEBUG linear_reconstruct] matrix[{i}] shape={m.shape}, rank/norm computation failed: {e}")
    ...
    bos_id = int(cfg.get("bos_token_id", getattr(ctx.tokenizer, "bos_token_id", 1) or 1))
    device = ctx.device
    ...
    try:
        ...
        first = generate_combinations([[bos_id]], token_pool, device)
        ...
        for ids, acts in _collect_candidates(ctx.global_model, first_batches, store, hook_module, device, 1):
            for layer_idx, matrix in enumerate(matrices[: acts.shape[0]]):
                ...
                for row, _col, error in hits:
                    layer_hits[layer_idx].append((ids[row].tolist(), error))
                if optimized:
        ...
        possible = [list(seq) for seq in dict.fromkeys(tuple(seq) for seq in first_tokens)]
    ...
    try:
        ...
        for chunk in batch_tensor(vocab, vocab_batch_size):
            ...
            for batch_ids, acts in _collect_candidates(ctx.global_model, [ids], store, hook_module, device, slice(None)):
                ...
                if matrix is None:
                    continue
                for row, col, _error in match_candidates(0, matrix, acts[0]):
                    if col < guess_len:
        ...
        for pos in range(max_len - 1):
            ...
            pending_prefixes = current_delayed + current
            total_combos = len(pending_prefixes) * len(next_tokens) if pending_prefixes else 0

            if pending_prefixes:
                all_combos = generate_combinations(pending_prefixes, next_tokens, device)
                hits_by_prefix: Dict[tuple, List[List[int]]] = {tuple(prefix): [] for prefix in pending_prefixes}
                offset = 0
                batch_idx = 0
                total_batches = (all_combos.shape[0] + combination_batch_size - 1) // combination_batch_size
                for ids, acts in _collect_candidates(ctx.global_model, batch_tensor(all_combos, combination_batch_size), store, hook_module, device, -1):
                    batch_idx += 1
                    ...
                    if matrix is None:
                        offset += ids.shape[0]
                        continue
                    hits = match_candidates(link_layer, matrix, acts[link_layer])
                    for row, _col, _error in hits:
                        seq = ids[row].tolist()
                        global_row = offset + row
                        prefix_idx = global_row // len(next_tokens)
                        if prefix_idx < len(pending_prefixes):
                            prefix_key = tuple(pending_prefixes[prefix_idx])
                            if prefix_key in hits_by_prefix:
                                hits_by_prefix[prefix_key].append(seq)
                    offset += ids.shape[0]

                for prefix in pending_prefixes:
                    prefix_key = tuple(prefix)
                    hits_for_prefix = hits_by_prefix.get(prefix_key, [])
