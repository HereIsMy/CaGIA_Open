...
def main() -> None:
    ...
    for config_path in args.config:
        ...
        methods = list(cfg.get("run", {}).get("methods", ["cagia-opt"]))
        total_runs = len(methods) * len(expanded_configs)
        run_idx = 0
        for method_name in methods:
            for base_cfg in expanded_configs:
                run_idx += 1
                run_cfg = copy.deepcopy(base_cfg)
                run_cfg.setdefault("run", {})["methods"] = [method_name]
                if total_runs > 1:
                    data_name = run_cfg.get("data", {}).get("name", "?")
                    batch_size = run_cfg["data"]["batch_size"]
                    peft_method = run_cfg.get("selected_peft", "?")
                    print(f"\nSweep {run_idx}/{total_runs}: method={method_name}, dataset={data_name}, batch_size={batch_size}, peft={peft_method}")
