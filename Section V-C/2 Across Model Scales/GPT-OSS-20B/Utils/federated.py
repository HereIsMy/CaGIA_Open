...
def _get_active_peft_config(cfg: Dict) -> Dict:
    ...
    model_name = cfg.get("model", {}).get("name") or cfg.get("model", {}).get("name_or_path")
    if model_name:
...
def run_fedsgd(cfg: Dict, device: torch.device) -> None:
    ...
    writer = JsonResultWriter(cfg.get("output", {}).get("path", "New/results/results.json"))
    ...
    for method_name in methods:
        print(f"\n{'=' * 60}")
        print(f"[Method] 开始执行方法: {method_name}")
        print(f"{'=' * 60}")
