...
class BottleneckAdapter(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.adapter_up(self.act(self.adapter_down(x)))
        return out.to(input_dtype)
...
def wrap_partial_modules(model: torch.nn.Module, target_path: str, layer_start: int, layer_end: int) -> None:
    target_path = resolve_target_module_path(model, target_path)
    layer_start, layer_end = resolve_layer_range(model, layer_start, layer_end)
    for block in iter_transformer_blocks(model)[layer_start:layer_end]:
        module = get_module_by_path(block, target_path)
        ...
        def forward_with_partial(self, hidden_states, *args, **kwargs):
            input_dtype = hidden_states.dtype
            hidden_states = hidden_states.to(torch.float32)
            output = self.forward_original(hidden_states, *args, **kwargs)
            if isinstance(output, torch.Tensor):
                return output.to(input_dtype)
            if isinstance(output, (tuple, list)):
                return type(output)(o.to(input_dtype) if isinstance(o, torch.Tensor) else o for o in output)
            return output

        module.forward = types.MethodType(forward_with_partial, module)
...
def configure_peft(model: torch.nn.Module, peft_cfg: Dict, num_labels: int) -> List[torch.nn.Parameter]:
    ...
    if method == "lora":
        ...
        wrap_partial_modules(model, resolved_target_path, layer_start, layer_end)
    elif method == "adapter":
...
def load_model_and_tokenizer(model_name: str, num_labels: int, peft_cfg: Dict, device: torch.device):
    import gc
    from transformers import AutoModel, AutoModelForCausalLM, AutoTokenizer, Mxfp4Config
    ...
    if "qwen" in model_name_lower or "llama" in model_name_lower or "gptoss" in model_name_lower or "gpt-oss" in model_name_lower:
    ...
    layer_end = int(peft_cfg.get("layer_end", -1))
    if layer_end > 0:
        blocks = iter_transformer_blocks(model)
        total = len(blocks)
        if layer_end < total:
            removed_blocks = blocks[layer_end:]
            _set_transformer_blocks(model, blocks[:layer_end])
            model.config.num_hidden_layers = layer_end
            if hasattr(model.config, "layer_types") and isinstance(model.config.layer_types, list):
                model.config.layer_types = model.config.layer_types[:layer_end]
            for block in removed_blocks:
                del block
            del removed_blocks, blocks
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

    remaining = len(iter_transformer_blocks(model))
...
def _set_transformer_blocks(model: torch.nn.Module, blocks: List) -> None:
    for attr in ["h", "layers", "transformer.h", "model.layers", "encoder.layer", "encoder.layers", "decoder.layers", "transformer.encoder.layers", "transformer.decoder.layers"]:
        node = model
        parts = attr.split(".")
        ok = True
        for part in parts[:-1]:
        ...
        if ok and hasattr(node, parts[-1]):
            setattr(node, parts[-1], torch.nn.ModuleList(blocks))
            return
    raise AttributeError("Cannot find transformer blocks to truncate; configure model_utils for this architecture.")
