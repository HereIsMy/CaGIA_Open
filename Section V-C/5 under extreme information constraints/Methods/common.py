...
def linear_reconstruct(
    ...
    store, handles = register_attention_input_hooks(ctx.global_model, hook_module, cfg.get("transformer_layer_index"))
    try:
        ...
        layer_hint = max(len(list(iter_transformer_blocks(ctx.global_model))), len(matrices), 1)
