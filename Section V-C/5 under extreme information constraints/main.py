...
def _expand_transformer_layer_configs(cfg: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    raw_layer = cfg.get("run", {}).get("transformer_layer_index")
    if not isinstance(raw_layer, list):
        yield cfg
        return
    for layer_index in raw_layer:
        expanded_cfg = copy.deepcopy(cfg)
        expanded_cfg.setdefault("run", {})["transformer_layer_index"] = (
...
def _expand_configs(cfg: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    for dataset_cfg in _expand_dataset_configs(cfg):
        for batch_cfg in _expand_batch_size_configs(dataset_cfg):
            for layer_cfg in _expand_transformer_layer_configs(batch_cfg):
                yield layer_cfg
