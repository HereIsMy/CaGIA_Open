...
def register_attention_input_hooks(model: torch.nn.Module, module_path: str, layer_index: int | None = None) -> Tuple[ActivationStore, List]:
    store = ActivationStore()
    handles = []
    blocks = list(iter_transformer_blocks(model))
    if layer_index is not None:
        layer_index = int(layer_index)
        if not 0 <= layer_index < len(blocks):
            raise IndexError(
                f"transformer layer index {layer_index} out of range: model has {len(blocks)} layers (0-{len(blocks) - 1})"
            )
        blocks = [blocks[layer_index]]
    for block in blocks:
