...
from .data import adjust_batch_to_fixed_length, collate_text_batch, load_dataset
...
def collect_batch_gradient(client_model, tokenizer, raw_batch, cfg: Dict, device: torch.device):
    ...
    fixed_length = cfg.get("data", {}).get("fixed_length")
    if fixed_length is not None:
        supplement_pool = cfg.get("_supplement_pool", [])
        pool_start = cfg.get("_supplement_index", 0)
        adjusted = adjust_batch_to_fixed_length(
            raw_batch["sentence"],
            tokenizer,
            int(fixed_length),
            supplement_pool,
            pool_start_idx=pool_start,
        )
        cfg["_supplement_index"] = adjusted["pool_end_idx"]
        tokenized = {
            "input_ids": adjusted["input_ids"],
            "attention_mask": adjusted["attention_mask"],
        }
        raw_batch = {**raw_batch, "sentence": adjusted["decoded_sentences"]}
    else:
        tokenized = tokenizer(
            raw_batch["sentence"],
            return_tensors="pt",
...
def run_attack_for_upload(method_name: str, global_model, upload, update_type: str, cfg: Dict, tokenizer, device: torch.device, writer: JsonResultWriter, round_idx: int, client_idx: int, batch_idx: int, raw_batch) -> None:
    ...
    if raw_batch is not None and "sentence" in raw_batch:
        ...
        fixed_length = cfg.get("data", {}).get("fixed_length")
        if fixed_length is not None:
            actual_lengths = [int(fixed_length)] * actual_batch_size
            actual_seq_len = int(fixed_length)
        else:
            max_length = cfg["data"].get("max_length")
            tokenized = tokenizer(
                sentences,
...
def run_fedsgd(cfg: Dict, device: torch.device) -> None:
    dataset = load_dataset(cfg)
    if cfg.get("data", {}).get("fixed_length") is not None:
        cfg["_supplement_pool"] = [row["sentence"] for row in dataset.rows]
        cfg["_supplement_index"] = 0
