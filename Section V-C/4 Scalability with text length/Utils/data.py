...
def adjust_batch_to_fixed_length(
    sentences: List[str],
    tokenizer,
    target_length: int,
    ...
    effective_target = max(target_length - num_special, 0)

    all_input_ids: List[List[int]] = []
    decoded_sentences: List[str] = []
    pool_idx = pool_start_idx
    pool_size = len(supplement_pool) if supplement_pool else 0

    for sentence in sentences:
        token_ids = tokenizer.encode(sentence, add_special_tokens=False)

        if len(token_ids) > effective_target:
            token_ids = token_ids[:effective_target]
        elif len(token_ids) < effective_target and pool_size > 0:
            while len(token_ids) < effective_target:
                extra = supplement_pool[pool_idx % pool_size]
                pool_idx += 1
                extra_ids = tokenizer.encode(extra, add_special_tokens=False)
                token_ids.extend(extra_ids)
            token_ids = token_ids[:effective_target]

        full_ids = tokenizer.build_inputs_with_special_tokens(token_ids)
        if len(full_ids) > target_length:
            full_ids = full_ids[:target_length]

        all_input_ids.append(full_ids)
        decoded_sentences.append(tokenizer.decode(full_ids, skip_special_tokens=True))
