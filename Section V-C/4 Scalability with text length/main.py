...
from Utils.results import recalculate_metrics_in_results
...
def _expand_fixed_length_configs(cfg: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    fixed_lengths = cfg.get("data", {}).get("fixed_length")
    if fixed_lengths is None:
        yield cfg
        return
    for length in _as_sweep_values(fixed_lengths):
        expanded_cfg = copy.deepcopy(cfg)
        expanded_cfg.setdefault("data", {})["fixed_length"] = int(length)
        yield expanded_cfg
...
def _expand_configs(cfg: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    for dataset_cfg in _expand_dataset_configs(cfg):
        for batch_cfg in _expand_batch_size_configs(dataset_cfg):
            for fl_cfg in _expand_fixed_length_configs(batch_cfg):
                yield fl_cfg
...
def main() -> None:
    ...
    for config_path in args.config:
        ...
        try:
            ...
            model_path = cfg.get("model", {}).get("name_or_path", "gpt2")
            _tokenizer = AutoTokenizer.from_pretrained(model_path)
        except Exception as e:
            print(f"[Warning] tokenizer 加载失败，mean_correct_tokens 将回退到单词级: {e}")
        updated = recalculate_metrics_in_results(results_path, tokenizer=_tokenizer)
        if updated > 0:
            pass
        ...
        for run_idx, run_cfg in enumerate(expanded_configs, start=1):
            if len(expanded_configs) > 1:
                ...
                fixed_length = run_cfg.get("data", {}).get("fixed_length")
                fl_info = f", fixed_length={fixed_length}" if fixed_length is not None else ""
                print(f"\nSweep {run_idx}/{len(expanded_configs)}: dataset={data_name}, batch_size={batch_size}{fl_info}")
