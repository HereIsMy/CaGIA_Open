...
def preprocess_matrix(matrix: torch.Tensor, eps: float = 1e-6, rank_tol: float = 1e-6) -> Dict[str, torch.Tensor | int]:
    a = matrix.detach().float()
    if a.ndim > 2:
        a = a.reshape(a.shape[0], -1)
    q, r = torch.linalg.qr(a, mode="reduced")
    singular_values = torch.linalg.svdvals(r.double()).float()
    max_sv = singular_values.max().clamp_min(torch.finfo(singular_values.dtype).tiny)
    rank = int((singular_values > rank_tol * max_sv).sum().item())
    if rank <= 0:
        return {"rank": 0}
    return {
        "rank": rank,
