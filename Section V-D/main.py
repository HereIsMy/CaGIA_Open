...
def _expand_method_param_configs(cfg: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    sweep_params: List[tuple] = []
    for method_name, method_cfg in (cfg.get("methods") or {}).items():
        if not isinstance(method_cfg, dict):
            continue
        for param_name, param_value in method_cfg.items():
            if isinstance(param_value, list):
                sweep_params.append((method_name, param_name, param_value))

    if not sweep_params:
        yield cfg
        return

    method_name, param_name, values = sweep_params[0]
    for value in values:
        expanded_cfg = copy.deepcopy(cfg)
        expanded_cfg["methods"][method_name][param_name] = value
        yield from _expand_method_param_configs(expanded_cfg)
...
def _expand_configs(cfg: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    for dataset_cfg in _expand_dataset_configs(cfg):
        for batch_cfg in _expand_batch_size_configs(dataset_cfg):
            for param_cfg in _expand_method_param_configs(batch_cfg):
                yield param_cfg
