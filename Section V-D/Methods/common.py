...
def linear_reconstruct(
    ...
    if peft_method in {"lora", "adapter"}:
        memory_estimate_multiplier *= 2.0
    ...
    try:
        ...
        if peft_method in {"lora", "adapter"}:
            try:
                blocks = iter_transformer_blocks(ctx.global_model)
                if blocks:
                    target_path = ctx.peft_config.get("resolved_target_module") or ctx.peft_config.get("target_module", hook_module)
                    target_module = get_module_by_path(blocks[0], target_path)
                    if hasattr(target_module, "nf"):
                        target_out_dim = int(target_module.nf)
                    elif hasattr(target_module, "out_features"):
                        target_out_dim = int(target_module.out_features)
                    elif hasattr(target_module, "weight") and target_module.weight.ndim >= 2:
                        target_out_dim = int(max(target_module.weight.shape))
                    else:
                        target_out_dim = hidden_hint
                    hidden_hint = max(hidden_hint, target_out_dim)
            except Exception:
                pass
