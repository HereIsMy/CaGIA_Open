...
def find_expressible_vectors3_preprocessed(b, preprocessed_data, tol_error=5e-3):
...
def find_expressible_vectors3(a, b, tol_error=1e-1):
...
def first_token_recon(attack_model, grade_data, com_batches):
    ...
    for com_batch in com_batches:
        ...
        for layer_index in range(len(attack_model.h)):
            results = find_expressible_vectors3_preprocessed(attn_inputs[layer_index], grade_data[layer_index], tol_error=6.5e-3)
...
def token_recon(attack_model, grade_data, com_batches):
    ...
    for index, com_batch in enumerate(com_batches):
        ...
        for layer_index in range(1, len(model.h)):
            results = find_expressible_vectors3_preprocessed(attn_inputs[layer_index], grade_data[layer_index], tol_error=5e-2)
            for re_index in range(len(results)):
...
def prune_smallest_1_percent(attn_grade, per=0.01):
    ...
    print(f"Pruning threshold (1% smallest): {threshold_value:.6e}")
...
def add_gaussian_noise(attn_grade, mean=0.0, std=0.01):
    noisy_tensors = []
    for t in attn_grade:
...
attn_inputs = []
for std in [0.01, 0.1, 1.5]:
    for batch_size in batch_sizes:
        print("Batch Size:", batch_size)

        result_file_save_path = os.path.join(base_dir, f'results/NAive_DP_{std}.json')
        ...
        for batch in train_loader:
            ...
            temp_cs = 4096
            while True:
                if (len(possible_token) > 0) or (len(possible_token_two) > 0):
                    current_possible_token = possible_token[:]
                    current_possible_token_two = possible_token_two[:]
                    possible_token = []
                    possible_token_two = []
                    if len(current_possible_token_two) > 0:
                        if len(current_possible_token_two[0]) < 5:
                            temp_cs = 4096
                        elif 4 < len(current_possible_token_two[0]) < 9:
                            temp_cs = 2048
                        else:
                            temp_cs = 1024
                        for temp_index, temp_possible_token in enumerate(current_possible_token_two):
