...
def find_expressible_vectors3_preprocessed(b, preprocessed_data, tol_error=100):
...
def first_token_recon(attack_model, grade_data, com_batches, token_index=0):
    global attn_inputs
    possible_token_function = [[] for _ in range(len(attack_model.h))]
    for com_batch in com_batches:
        attn_inputs = []
        with torch.no_grad():
            _ = attack_model(input_ids=com_batch.clone().detach().to(device))
        attn_inputs = [tensor[:, token_index, :] for tensor in attn_inputs]
        attn_inputs = torch.stack(attn_inputs, dim=0).unsqueeze(2)
        for layer_index in range(len(attack_model.h)):
            results = find_expressible_vectors3_preprocessed(attn_inputs[layer_index], grade_data[layer_index], tol_error=100)
            results_temp = []
            for re_index in range(len(results)):
                results_temp.append((com_batch[results[re_index][0], results[re_index][1]].item(), results[re_index][2]))
            possible_token_function[layer_index].extend(results_temp)
    return possible_token_function
...
def token_recon(attack_model, grade_data, com_batches):
    ...
    for index, com_batch in enumerate(com_batches):
        attn_inputs = []
        with torch.no_grad():
            _ = attack_model(input_ids=com_batch.to(device))
        ...
        for layer_index in range(1, len(model.h)):
            ...
            results_temp = []
            for re_index in range(len(results)):
                results_temp.append((com_batch[results[re_index][0]].tolist(), results[re_index][2]))
            possible_token_function[layer_index].extend(results_temp)
    return possible_token_function


def find_top_k_min_errors(data, k=3):
    sorted_data = sorted(data, key=lambda x: x[1])
    return sorted_data[:k]


def extract_tokens_with_large_ratio(top_k_errors, threshold=5.0):
    token = []
    for idx, error_list in enumerate(top_k_errors):
        errors = [error for _, error in error_list]
        ...
        for k in range(max_index + 1):
            token.append(error_list[k][0])
    return token


def extract_tokens_with_large_ratio1(top_k_errors, threshold=5.0):
    token = []
    for idx, error_list in enumerate(top_k_errors):
        errors = [error for _, error in error_list]
        ratios = [errors[i] / errors[i - 1] for i in range(1, len(errors))]
...
def add_gaussian_noise(attn_grade, mean=0.0, std=0.0001):
    noisy_tensors = []
    for t in attn_grade:
        noise = torch.randn_like(t) * std**2 + mean
        noisy_t = t + noise
...
for std in std_values:
    print(f"\nRunning Opt_DP with std = {std}")
    for batch_size in batch_sizes:
        print("Batch Size:", batch_size)

        result_file_save_path = os.path.join(base_dir, f'results/Opt_DP_{std}.json')
        ...
        for batch in train_loader:
            ...
            attn_grade = add_gaussian_noise(attn_grade, std=std)

            a_result = []
            for layer_index in range(len(model_new.h)):
                a_result.append(preprocess_a(attn_grade[layer_index][:, 768*2:(768*3-5)]))

            recon_sentence_one = []

            combination = generate_combination(Token_index_effective_temp, [[50256]])
            com_batches = generate_batch(8192, combination)
            com_and_errors = first_token_recon(model, a_result, com_batches)
            top_k_errors = []
            for temp_index in range(len(com_and_errors)):
                top_k_errors.append(find_top_k_min_errors(com_and_errors[temp_index], k=200))
            token = extract_tokens_with_large_ratio(top_k_errors, threshold=threshold)
            token = list(dict.fromkeys(token))
            possible_token = [[temp_token] for temp_token in token]

            temp_cs = 4096
            while 3 > len(possible_token) > 0:
                ...
                for temp_index, temp_possible_token in enumerate(current_possible_token):
                    ...
                    com_and_errors = token_recon(model, a_result, com_batches)
                    top_k_errors = []
                    for temp_index in range(len(com_and_errors)):
                        top_k_errors.append(find_top_k_min_errors(com_and_errors[temp_index], k=200))
                    new_temp_possible_token.extend(extract_tokens_with_large_ratio1(top_k_errors, threshold=threshold))
                    new_temp_possible_token = list(set(tuple(token) for token in new_temp_possible_token))
                    new_temp_possible_token = [list(token) for token in new_temp_possible_token]
                    if len(new_temp_possible_token) == 0:
                        recon_sentence_one.append(temp_possible_token)
                    else:
                        possible_token.extend(new_temp_possible_token)
                possible_token = list(set(tuple(token) for token in possible_token))
                possible_token = [list(token) for token in possible_token]

            recon_sentence_one.extend(possible_token)
